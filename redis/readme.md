...
hash
　　这个是类似map的一种结构，这个一般就是可以将结构化的数据，比如一个对象（前提是这个对象没嵌套其他的对象）给缓存在redis里，然后每次读写缓存的时候，可以操作hash里的某个字段。
...
list
　　列表，比如微博某大v的粉丝，就可以以list的格式放在redis里去缓存
...
　　可以基于set使用交集、并集、差集的操作，比如交集，可以把两个人的粉丝列表做一个交集，看看俩人的共同好友是谁
